...
export default function ApprovedDocumentsPage() {
  ...
  const fetchHistorique = async () => {
    ...
    try {
      ...
      setHistoriqueData(res.data.historique || []);
    } catch {
      setHistoriqueData([]);
    } finally {
    ...
    <div className="min-h-screen w-full flex flex-col justify-between bg-gray-50">
              ...
              <div className="flex flex-row items-center gap-2 mb-0">
                <Dialog open={showHistorique} onOpenChange={setShowHistorique}>
                  ...
                  <DialogContent className="max-w-2xl">
                      ...
                      <table className="w-full border text-xs">
                        <thead>
                          <tr className="bg-gray-100">
                            ...
                            <th className="px-2 py-2">Total à payer</th>
                        ...
                        <tbody>
                          {historiqueData.map((row, idx) => (
                            <tr key={row.mois} className="border-b">
                              ...
                              <td className="px-2 py-2">{row.totalAPayer.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</td>
                              <td className="px-2 py-2">
                                <Button size="sm" onClick={() => setSelectedMonthIdx(idx)}>
                                  Voir
                                </Button>
                              </td>
                            </tr>
                    ...
                    {/* Show documents for selected month */}
                          ...
                          <tbody>
                            {historiqueData[selectedMonthIdx]?.docs.map((doc, i) => (
                              <tr key={doc.docId || i} className="border-b">
                                <td className="px-2 py-2">{doc.docId || '-'}</td>
                                <td className="px-2 py-2">{doc.date ? formatDateFr(doc.date) : '-'}</td>
                                <td className="px-2 py-2">{doc.total ? doc.total.toLocaleString('fr-FR', { minimumFractionDigits: 2 }) : '-'}</td>
                                <td className="px-2 py-2">{doc.status || 'N/A'}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                        <div className="mt-2 text-right">
                          <Button size="sm" variant="outline" onClick={() => setSelectedMonthIdx(null)}>
                            Retour
                          </Button>
                        </div>
                      </div>
      ...
      <Dialog open={showHistorique} onOpenChange={setShowHistorique}>
        ...
        <DialogContent className="max-w-2xl">
          ...
          {selectedMonthIdx !== null && (
            <div className="mt-6">
              <h3 className="font-semibold mb-2">Documents pour {historiqueData[selectedMonthIdx]?.mois}</h3>
              <table className="w-full border text-xs">
                <thead>
                  <tr className="bg-gray-100">
                    <th className="px-2 py-2">Document</th>
